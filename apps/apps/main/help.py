...
from bot_cps.config import PATH as BOT_CPS_PATH
...
CMD_LIST: list[str] = sorted(
    list(map(lambda path: basename(path),
             glob(f"{PATH.DOCS_TITLE}/en-US/*") + glob(f"{BOT_CPS_PATH.DOCS_TITLE}/en-US/*")))
)
    ...
    def generate_docs(self) -> None:
        ...
        for cmd in CMD_LIST:
            ...
            for locale in Locale:
                if exists(f"{BOT_CPS_PATH.DOCS_TITLE}/{locale}/{cmd}"):
                    with open(f"{BOT_CPS_PATH.DOCS_TITLE}/{locale}/{cmd}", "r") as f:
                        title_arg.update({locale: f.read()})
                if exists(f"{PATH.DOCS_TITLE}/{locale}/{cmd}"):
                    with open(f"{PATH.DOCS_TITLE}/{locale}/{cmd}", "r") as f:
                        title_arg.update({locale: f.read()})

                if exists(f"{BOT_CPS_PATH.DOCS_DETAIL}/{locale}/{cmd}"):
                    with open(f"{BOT_CPS_PATH.DOCS_DETAIL}/{locale}/{cmd}", "r") as f:
